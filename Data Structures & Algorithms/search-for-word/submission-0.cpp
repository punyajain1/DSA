...
public:
    bool searchNext(vector<vector<char>> &board, string word, int row, int col, int index, int m, int n) {
        ...
        board[row][col] = '!'; // when we do recursion as seen below , we should encounter same word again so to prevent this we replace it with special char to prevent it from loop as when we encounter it again we will simplay return false as we cant choose same again
